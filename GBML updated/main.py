...
from torchmeta.datasets import MiniImagenet, DoubleMNIST
...
def train(args, model, dataloader):
    ...
    with tqdm(dataloader, total=args.num_train_batches) as pbar:
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            global train_iter
            writer.add_scalar('Accuracy/train', round(acc_log), train_iter)
            writer.add_scalar('Loss/train', round(loss_log), train_iter)
            train_iter += 1
...
def valid(args, model, dataloader, test):
    ...
    with tqdm(dataloader, total=args.num_valid_batches) as pbar:
        for batch_idx, batch in enumerate(pbar):
            ...
            if test:
                global test_iter
                writer.add_scalar('Accuracy/test', round(acc_log), test_iter)
                writer.add_scalar('Loss/test', round(loss_log), test_iter)
                test_iter += 1
...
def main(args):
    ...
    train_dataset = DoubleMNIST(args.data_path, num_classes_per_task=args.num_way,
                                meta_split='train',
    ...
    valid_dataset = DoubleMNIST(args.data_path, num_classes_per_task=args.num_way,
                                meta_split='val',
                                transform=transforms.Compose([
    ...
    test_dataset = DoubleMNIST(args.data_path, num_classes_per_task=args.num_way,
                               meta_split='test',
                               transform=transforms.Compose([
                                   transforms.CenterCrop(80),
                                   transforms.ToTensor(),
...
def parse_args():
    ...
    parser.add_argument(
        '--load_encoder', type=lambda x: (str(x).lower() == 'true'), default=False)
    ...
    # Added flags
    parser.add_argument('--TTSA', type=bool, default=False)
    parser.add_argument('--logfile', type=str,
                        help='log file name inside logs/ folder', default='logs.txt')
    parser.add_argument('--imaml_reg', type=bool, default=False)
    parser.add_argument(
...
if __name__ == '__main__':
    ...
    set_gpu(args.device)
    check_dir(args)
    set_logger(os.path.join('logs/', args.logfile), log_console=False)
    main(args)
