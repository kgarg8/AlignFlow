...
def dense(in_dim, out_dim, hidden_dim, n_dense):
    ...
    layers[::2] = [nn.Linear(shapes[i], shapes[i + 1])
...
class ConvNet(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.encoder = [conv3x3(self.in_channels, self.hidden_channels, True)] + [conv3x3(
            self.hidden_channels, self.hidden_channels, i < 3) for i in range(args.n_conv - 1)]
        self.encoder = nn.Sequential(*self.encoder)
        self.decoder = dense(self.hidden_channels * 5 * 5,
                             self.out_features, args.hidden_dim, args.n_dense)
        self.init_params()
        return None
